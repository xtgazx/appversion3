...
export function TaskCard({
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-2xl border border-slate-200 px-3 py-3 dark:border-slate-700 ${task.done ? "bg-slate-50 opacity-80 dark:bg-slate-900" : "bg-white dark:bg-slate-800"}`}
    >
      <button type="button" onClick={onToggle} className="mt-0.5">
        <CheckCircle2
          className={`h-5 w-5 ${task.done ? "text-emerald-600" : "text-slate-300 dark:text-slate-500"}`}
        />
      </button>
      ...
      <div className="flex-1">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <InlineText
              value={task.title}
              onSave={onSaveTitle}
              displayClassName={`w-full rounded-xl px-1 py-1 text-left text-sm transition hover:bg-slate-50 dark:hover:bg-slate-700/60 ${task.done ? "text-slate-400 line-through" : "text-slate-900 dark:text-slate-100"}`}
            />
          </div>
        </div>
        ...
        <div className="mt-1 flex flex-wrap gap-2">
          ...
          {projectName ? (
            <MetaPill>{projectName}</MetaPill>
          ) : (
            <MetaPill>One-off Task</MetaPill>
          )}
        </div>
